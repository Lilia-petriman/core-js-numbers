...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const sideOne = Math.abs(x1 - x2);
  const sideTwo = Math.abs(y1 - y2);
  const distance = Math.sqrt(sideOne ** 2 + sideTwo ** 2);
  return distance;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const magnt1 = Math.sqrt(x1 * x1 + y1 * y1);
  const magnt2 = Math.sqrt(x2 * x2 + y2 * y2);
  const cosineAngle = dotProduct / (magnt1 * magnt2);
...
function roundToPowerOfTen(num, pow) {
  const numberFraction = num / 10 ** pow;
  const numRounded = Number(numberFraction.toFixed());
  return numRounded * 10 ** pow;
}
...
function toNumber(value, def) {
  ...
  return Number.isFinite(valueNum) ? valueNum : def;
}
...
function getFibonacciNumber(index) {
  ...
  const number = g ** index / Math.sqrt(5);
...
function toExponential(number, fractionDigits) {
  const exp = Number.parseFloat(number).toExponential(fractionDigits);
  return exp;
}
